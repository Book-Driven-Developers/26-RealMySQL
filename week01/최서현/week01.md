...
> **(1) 왜 Query Parser가 필요한가**
> 

MySQL은 사람이 작성한 SQL 문자열을 그대로 실행할 수 없다.

DB는 문자열이 아니라 **구조화된 명령**이 필요하다.

따라서 MySQL은 SQL을 실행하기 전에 다음 과정을 거친다.

```
SQL 문자열 → 파싱 → 내부 트리 구조 생성 → Optimizer 전달
```

이 과정이 바로 **Query Parser의 역할**이다.

> **(2) 전체 쿼리 처리 흐름에서 Parser 위치**
> 

MySQL의 쿼리 처리 흐름은 다음과 같다.

```
Client → Parser → Optimizer → Executor → Storage Engine
```

Parser는 **Optimizer 이전 단계**에서 동작하며, SQL이 “문법적으로 올바른지” 확인하고 내부 표현으로 변환한다.

> **(3) Parser는 두 단계로 동작한다**
> 

Query Parser는 크게 두 단계로 구성된다.
...
- **Lexical Analysis (토큰화)**
    ...
    - 토큰화 결과 (개념적 표현)
        ...
        ```sql
        SELECT | IDENT(name) |FROM | IDENT(users) |WHERE | IDENT(id) | = | NUMBER(1)
        ```
        
    - 이 단계에서는 단순히 문자열을 “의미 있는 단어 단위”로 나눈다.
    - 역할 요약
        - 키워드 인식 (SELECT, FROM, WHERE)
        - 식별자 인식 (테이블명, 컬럼명)
        - 숫자/문자 리터럴 인식
    - 이 단계는 컴파일러의 **Lexer**와 동일한 역할이다.
- **Syntax Analysis (문법 분석)**
    - 토큰들을 이용해 SQL 문장이 **문법 규칙에 맞는지 검사**하고 트리를 생성한다.
    - MySQL 내부에는 SQL 문법이 정의된 **Grammar 규칙**이 존재한다.
    - 예시 규칙 (개념적)
    ...
    - 토큰이 문법에 맞지 않으면 이 단계에서 에러가 발생한다.
    - 대표적인 예
...
> **(4) Parser가 문법 오류를 검출하는 원리**
> 

MySQL이 쿼리 실행 전에 문법 오류(Syntax Error)를 반환할 수 있는 이유는

Parser가 SQL을 **문법 규칙(Grammar)** 기반으로 분석하기 때문이다.

MySQL 내부에는 SQL 문장의 구조가 정식 문법 형태로 정의되어 있다.

예를 들어 SELECT 문은 다음과 같은 형태를 따라야 한다.
...
Parser의 목표는 토큰들을 이용해 이 문법 규칙에 맞는 **Parse Tree를 생성하는 것**이다.

이때 중요한 기준은 다음과 같다.

```
트리 생성 가능 → 문법적으로 올바른SQL
트리 생성 실패 → 문법 오류 발생
```
...
→ column_list가 없어 문법 규칙 위반

→ Parse Tree 생성 실패

→ Parser 단계에서 Syntax Error 발생

Parser는 Optimizer보다 먼저 실행되기 때문에,

문법 오류가 있는 쿼리는 실행 단계까지 도달하지 못하고 즉시 오류가 반환된다.

> **(5) Parse Tree (또는 AST) 생성**
> 

문법 분석이 끝나면 MySQL은 SQL을 **트리 구조**로 변환한다.

- 예시 SQL

```
SELECTnameFROM usersWHERE id =1;
```
...
이 트리는 사람이 읽는 SQL이 아니라

MySQL 내부에서 처리하기 위한 **구조화된 명령**이다.

이 트리가 이후 단계인 Optimizer로 전달된다.

> **(6) Parser 단계에서 수행되는 중요한 역할**
> 

Parser는 단순 문법 검사 이상의 역할을 수행한다.
...
- 테이블/컬럼 이름 추출
- WHERE 조건 구조 분석
- 쿼리 타입 판별 (SELECT, INSERT, UPDATE 등)

즉, Optimizer가 실행계획을 세울 수 있도록
...
MySQL Query Parser는 SQL 실행의 첫 단계로서 다음 역할을 수행한다.

1. SQL 문자열을 토큰으로 분해한다.
2. 문법 규칙을 기반으로 Syntax를 검사한다.
3. SQL을 트리 구조(Parse Tree)로 변환한다.
4. 이 결과를 Optimizer에 전달한다.

즉, Query Parser는 SQL을 사람이 읽는 문장에서

DB가 이해할 수 있는 내부 구조로 변환하는 **컴파일러의 프론트엔드 역할**을 수행한다.